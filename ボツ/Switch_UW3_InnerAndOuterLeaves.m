function [Zerr,Xerr] = Switch_UW3_InnerAndOuterLeaves(L0, sigGKP, etas, etam, etad, etac, Lcavity, k, v, leaves, N)
...
[Zerr, Xerr] = Switch_UW3_InnerAndOuterLeaves(9, 0.12, 0.995, 0.999995, 0.9975, 0.99, 2, 15, 0.3, 0, 10000000)
...
Zerr_squared = ZerrInnerVec.*(ones(k,1) - ZerrOuter) + ZerrOuter.*(ones(k,1) - ZerrInnerVec);
Xerr_squared = XerrInnerVec.*(ones(k,1) - XerrOuter) + XerrOuter.*(ones(k,1) - XerrInnerVec);

%Zerr = sqrt(Zerr_squared);
%Xerr = sqrt(Xerr_squared);
