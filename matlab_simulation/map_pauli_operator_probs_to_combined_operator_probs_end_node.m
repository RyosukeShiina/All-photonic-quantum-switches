function combinedPauliOperatorResults = map_pauli_operator_probs_to_combined_operator_probs_end_node(ghzPauliOperatorResults)


% Maps individual end-node GHZ Pauli-operator probabilities to the
% combined three-qubit Pauli-operator probability distribution.
%
...
persistent sourceMasks pauliOperatorLabels numSourceOperators


if isempty(sourceMasks)

    pauliTable = ghzPauliOperatorResults.PauliOperators;

    [numSourceOperators, numQubits] = size(pauliTable);

    if numSourceOperators ~= 28 || numQubits ~= 3
        error('End-node PauliOperators must contain a 28-by-3 Pauli table.');
    end
    ...
    for sourceIndex = 1:numSourceOperators
        ...
        for qubitIndex = 1:3
            ...
            pauliOperator = ...
                upper(string(pauliTable(sourceIndex, qubitIndex)));
            ...
            switch pauliOperator
                ...
                otherwise
                    error('Unsupported source Pauli operator: %s', ...
                        pauliOperator);
            end
        end

        sourceMasks(sourceIndex) = uint8( ...
              xMask(1) * 1 ...
            + xMask(2) * 2 ...
            + xMask(3) * 4 ...
            + zMask(1) * 8 ...
...
if length(sourceProbs) ~= numSourceOperators
    error('Expected %d source probabilities, but received %d.', ...
        numSourceOperators, length(sourceProbs));
end
...
for sourceIndex = 1:numSourceOperators
    ...
    if ~isscalar(sourceProb) || ...
            ~isnumeric(sourceProb) || ...
            ~isfinite(sourceProb)
        error('Each source probability must be a finite numeric scalar.');
    end

    if sourceProb < 0 || sourceProb > 1
        error('Each source probability must be in the interval [0, 1].');
    end
    ...
    for operatorIndex = 0:63
        ...
        updatedCombinedProbs(operatorIndex + 1) = ...
            updatedCombinedProbs(operatorIndex + 1) ...
            + (1 - sourceProb) * currentProb;


        % Case 2: this source Pauli operator occurs.
        newOperatorIndex = ...
            bitxor(uint8(operatorIndex), sourceMask);

        updatedCombinedProbs(double(newOperatorIndex) + 1) = ...
            updatedCombinedProbs(double(newOperatorIndex) + 1) ...
            + sourceProb * currentProb;
    end
